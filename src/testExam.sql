...
alter  table  customers rename column  nationally to nationality;
...
select  max(first_name) , house_type from  owners  join houses h on owners.id = h.owner_id group by  house_type limit 1;
...
select  address_id, houses from  houses join addresses a on a.id = houses.address_id where address_id in (5,6,7,8,9,10);
-- 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select  houses,c.first_name,o.first_name,name from   houses join rent_info ri on houses.id = ri.house_id join owners o on o.id = houses.owner_id join customers c on c.id = ri.customer_id join agencies a on houses.address_id = a.address_id;
...
select  house_type,rating, o.* from  houses join owners o on o.id = houses.owner_id order by  rating;
select  house_type,rating, o.* from  houses join owners o on o.id = houses.owner_id order by  rating desc ;
...
select   *, house_type     from  agencies join houses h on agencies.address_id = h.address_id where name ='My House';
-- 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select  first_name, * from  addresses join rent_info ri on addresses.id = ri.agency_id join owners o on o.id = ri.owner_id;
-- 10.Кленти жок уйлордун баарын жана анын адрессин жана ал уйлор кайсыл агентсвога тийешелуу экенин чыгар.
select  * ,a.*,a2.* from  houses join rent_info ri on houses.id = ri.house_id join addresses a on a.id = houses.address_id join agencies a2 on a.id = a2.address_id;
-- 11.Клиенттердин улутуна карап, улутуну жана ал улуутта канча клиент жашайт санын чыгар.
select customers.nationally, count(*) from  customers group by  nationally;
...
select  o.first_name,customers.first_name from  customers join public.rent_info ri on customers.id = ri.customer_id join owners o on o.id = ri.owner_id;
-- 16.Эн уйу коп owner_ди жана анын уйлорунун санын чыгар.


-- 17.Улуту Kyrgyzstan уй-булолу customerлерди чыгарыныз.
select  * from  customers where  customers.nationality ='Kyrgyzstan' and marital_status ='Married';
-- 18.Эн коп болмолуу уйду жана анын адресин ал уй кайсыл ownerге таандык ошону чыгарыныз.
select  max(room) from  owners join public.houses h on owners.id = h.owner_id join addresses a on a.id = h.address_id;
-- 19.Бишкекте   жайгашкан уйлорду  жана  клиентерин  кошо чыгарыныз.
select  h.*,customers.* from  customers join rent_info ri on customers.id = ri.customer_id join houses h on h.id = ri.house_id join addresses a on a.id = h.address_id where city = 'Bishkek';
...
select  h.* from  rent_info join houses h on h.id = rent_info.house_id where  rent_info.check_in
-- 22.Эн кымбат уйду жана анын ээсин чыгарыныз.
select  max(price) from  houses join rent_info ri on houses.id = ri.house_id join customers c on c.id = ri.customer_id;
